...
function buildCharts() {
  ...
  // Chart 2 — Origen de leads (doughnut)
  const ctxOrigen = document.getElementById('chart-origen');
  if (ctxOrigen) {
    new Chart(ctxOrigen, {
      type: 'doughnut',
      data: {
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: true,
            position: 'bottom',
            labels: { color: TICK_COLOR, font: { size: 10 }, padding: 12, boxWidth: 10 }
          }
        }
      }
    });
  ...
  // Chart 3 — Conversión por etapa (horizontal bar)
  const ctxConv = document.getElementById('chart-conv');
  if (ctxConv) {
    new Chart(ctxConv, {
      type: 'bar',
      data: {
        labels: ['Nuevo','Calificado','Cita','Cerrado'],
        datasets: [{
          data: [100, 64, 36, 18],
      ...
      options: {
        ...
        plugins: { legend: { display: false } },
        scales: {
          x: { grid:{ color:GRID_COLOR }, ticks:{ color:TICK_COLOR, font:{size:10} }, beginAtZero:true },
